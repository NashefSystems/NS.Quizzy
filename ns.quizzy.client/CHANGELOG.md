...
- Create Angular project with ng: `ng new ns.quizzy.client --defaults --skip-install --skip-git --no-standalone `.
...
- Create project file (`ns.quizzy.client.esproj`).
- Create `launch.json` to enable debugging.
- Create `nuget.config` to specify location of the JavaScript Project System SDK (which is used in the first line in `ns.quizzy.client.esproj`).
