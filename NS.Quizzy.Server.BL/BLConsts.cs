namespace NS.Quizzy.Server.BL
{
    internal static class BLConsts
    {
        internal const string AUTH_TOKEN_KEY = "_qat";
        internal const string APP_SETTING_SECURITY_KEY = "DExJoqaJjHkLYC2XnpnHa8W8LpFt2qE3";

        internal const string QUEUE_EXAM_EVENTS = "exam-events";
        internal const string QUEUE_UPDATE_USERS = "update-users";
        internal const string QUEUE_PUSH_NOTIFICATIONS = "push-notifications";
        internal const string QUEUE_VIRTUAL_HOST = "/ns-quizzy";
        internal const int MESSAGE_STATUS_CACHE_TTL_IN_DAYS = 10;

    }
}
